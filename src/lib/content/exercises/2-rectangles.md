...
Notice the pattern: `x` stays at `0` and `height` stays at `50`, while each bar's `y` jumps by `70` (50 for the bar + 20 for the gap).
...
- The first bar with value = 50%. (_hint: this will be 50% or half of the chart width. So 800 / 2 = 400_)
- Second bar = 75%.
- Third bar = 33%.
- Fourth bar = 20%.
